...
local function grok_string(self, text, start, etc)
   ...
   if text:sub(start,start) ~= '"' then
	  ...
	  return {error="expected string's opening quote"}
   end
...
local function grok_object(self, text, start, etc)
   ...
   while i <= text_len do
      ...
      if text:sub(i, i) ~= ',' then
		 ...
		 return {error="expected comma or '};"}
      end
...
function OBJDEF:decode(text, etc)
   if type(self) ~= 'table' or self.__index ~= OBJDEF then
	  ...
	  return {error="JSON:decode must be called in method format"}
   end

   if text == nil then
      --self:onDecodeOfNilError(string.format("nil passed to JSON:decode()"), nil, nil, etc)
	  return {error="nil passed to JSON:decode()"}
   elseif type(text) ~= 'string' then
      --self:onDecodeError(string.format("expected string argument to JSON:decode(), got %s", type(text)), nil, nil, etc)
	  return {error="expected string argument to JSON:decode(), got %s"}
   end
   ...
   if text:match('^%s*<') then
	  ...
	  return {error="html passed to JSON:decode()"}
   end
   ...
   if text:sub(1,1):byte() == 0 or (text:len() >= 2 and text:sub(2,2):byte() == 0) then
	  ...
	  return {error="JSON package groks only UTF-8, sorry"}
   end
